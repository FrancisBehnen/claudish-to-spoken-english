...
**ROUND 34 SWEPT THE SAME DIRECTORY AGAIN AND FOUND THE SWEEP ITSELF WAS TOO WEAK, AT SIX SITES.**
Every one of the refusals above was written as *does this input exist* — `-f`, `-e`, `-f && -r`, or
a count of rows or of distinct ids. **None of them establishes that the input contains anything**,
and a zero-byte file passes all of them. So the whole class came back a third consecutive round:
`lock_overlap.sh` counted an empty trace in the shell glob and never dispatched on it, reporting
`traces read=4` over seven traces and exiting 0; `analyse_c14.sh` classified 12 of 12 trials as
`nohook`, satisfied its own accounting total and printed *0 unreachable, 0 reachable*;
`analyse_round2.sh` printed `0 of 0` from an empty `C13a` trace; `collect.sh` **wrote** an evidence
row with no rc, no player interval and no hook result; `run_lock.sh` validated no argument at all;
and `assemble_pass1.sh` counted **distinct** trial ids and called that the trial **set**, so
`1..11,13` passed with trial 12 missing. The rule now has **one definition**, in `require.sh`, and
seven scripts call it — which is the third time this rig has answered a many-copies rule that way
(`attrib.sh`, `cleanup.sh`, `require.sh`), and the reason is the same each time: a rule with six
copies gets fixed at one copy per round.

**AND BOTH ASSEMBLERS TRUNCATED THE COMMITTED EVIDENCE BEFORE COLLECTING IT.** `assemble.sh` opened
with `: > "$DEST/preemption-trials.tsv"` and `assemble_pass1.sh` with the same line for the
replication arm, so every `exit 2` after that point — a failed `collect.sh`, a missing `trials.tsv`,
a short row count, each of them a guard a previous round *added* — left the destination holding a
partial or zero-byte file where valid committed evidence had been. Measured against the real file,
the pre-fix script turns a 313-line `preemption-trials.tsv` into **0 lines** and exits 2. **Nothing
was ever lost**: the truncation has been in `assemble.sh` since `9e5a29b`, the commit that first
published the evidence, so it is an ancestor of every write and *could* have run — but no committed
state of either TSV is shorter than its predecessor (133 → 313 → 313, and 133 → 133; `c293618` is a
one-line header rename with byte-identical data rows). Both now assemble into a staging file and
`mv` it into place only after every collection and row-count check has passed; `mv` within one
directory is `rename(2)`, which replaces in one step, so a failure before it leaves the committed
...
**AND "REAPED" WAS A WORD THIS FILE HAD NOT EARNED UNTIL ROUND 33 — harness defect 8.**
`reap_stray_players` sent SIGTERM, counted the `kill` calls and returned success. `kill(2)` only
queues a signal, so that count established nothing about whether a player had stopped: measured
here, `player_probe.py` is alive when `kill(SIGTERM)` on it returns in **20 of 20** trials, for
0.50–0.99 ms, because its handler writes and `fsync`s a `player_end` record before restoring
`SIG_DFL` and re-raising. The reason that matters is **not** the sub-millisecond window — the
review finding that raised it said the wrapper *"may start the next configuration immediately"*, and
it may not: every wrapper here `exit 2`s on a driver failure, and on a *successful* run
`run_preempt.sh`'s trial-ending `sleep 6` and `run_real.sh`'s `sleep 12` outlive their 2.5 s and
5.7 s players, so nothing is signalled on the only path a wrapper continues down. The reason is
**harness defect 1**, which is on this list because `nohup` had set the swept signal to `SIG_IGN`,
`killpg` returned **success**, nothing died, and the rig produced a false 12/12. A signaller that
reports what it sent cannot tell that from a clean reap — and this is the *leak check*, so
"signalled" being read as "gone" is the whole of its value. It now polls the signalled set for a
bounded 2000 ms, escalates survivors to **SIGKILL** (which no disposition can ignore) and polls
1000 ms more, and reports **reaped** and **survived** as separate numbers, with a `LEAK` block
naming any pid alive after that. **No committed figure could have come from it**: `cleanup.sh` was
added in round 30 and the committed traces were last written by an ancestor of that commit, so
this function has never run in a run that produced published evidence.
...
- **ROUND 33: THIS SCRIPT COULD PASS WITHOUT CHECKING ANYTHING, ON TWO PATHS, AND IT IS ONE
  COMMIT OLD.** Both are the null-as-pass shape the decision document's §1 keeps meeting, and this
  is the first time that shape has appeared **twice in one file** (derivation defect 19).
  - **The coverage fraction above was printed and not enforced.** The expected trace count was an
    optional second argument under a `> 0` test, so `lock_overlap.sh traces` — the invocation this
    table gives and the document quotes — never ran it, and **deleting one of the seven committed
    traces still exited 0** while the fraction went on saying 7. The committed set's size is now
    the default; a non-numeric count is refused rather than silently evaluated as zero; `0` still
    disables the check but announces it on stderr.
  - **A run with no two-owner trace reported success.** Concurrency is only testable where a trial
    has two owners, and where none did the script printed *"nothing was checked here"* and
    **exited 0** — the one guard cited as establishing concurrent tenure, reporting establishment
    from a run that established nothing. Zero qualifying trials is now exit 2, and so is a set of
    files that parse to no records, which the empty-glob guard did not catch.

  **No figure moves**: over the committed traces with no argument it still reads 7 traces, 4
  two-owner, concurrency proven on 4, exit 0. **Both refusal modes are demonstrated in six
  directions** — a trace deleted, all traces single-owner, an empty directory (with and without the
  count check disabled), seven zero-byte traces, a non-numeric count — against two acceptance
  directions, the committed set and a two-trace subset with its count passed explicitly.
...
| `require.sh` | **the null-as-pass rule, sourced not executed — new in round 34.** *"This file exists"* is not *"this file contains anything"*, and by round 34 that check had **six independent implementations** in this rig — `-f`, `-e` and `-f && -r` at four consumers, and a driver plus an assembler that validated nothing at all — while the phrase `null-as-pass` had been in four of these scripts since round 12. A zero-byte file passes every one of those tests and reaches `awk` as a name with no records: `lock_overlap.sh` counted it in the shell glob and never dispatched on it (`FNR == 1`), so **seven traces of which three were empty reported `traces read=4` and exited 0** — a third hole in round 33's repair of the same guard; `analyse_c14.sh` classified 12 of 12 trials as `nohook`, satisfied its own accounting total and printed *0 unreachable, 0 reachable*; `analyse_round2.sh` printed `0 of 0` from an empty `C13a`; and `collect.sh` **wrote** an evidence row with no rc, no player interval and no hook result. Seven consumers now source it and each **refuses to run** if it is missing or defines nothing, for the reason `attrib.sh`'s consumers do. It holds `require_dir`, `require_nonempty`, `require_nonempty_all`, `require_uint`, `require_data_rows` and `require_exact_id_set` — the last because a count of **distinct** ids is not the id **set**: `1..11,13` has twelve distinct values with trial 12 missing, and it tests membership with array subscripts rather than `==`, since a trailing U+2032 PRIME is invisible to this `awk`'s comparison operators and visible to a subscript. See derivation defects 20 and 21 |
| `cleanup.sh` | **the drivers' fatal-path cleanup, sourced not executed — new in round 30.** The round-27 `EXIT` trap terminated only `$WPID`, and the worker's players are not the worker: `os.setsid()` makes the worker a group leader, so a pid kill left `afplay` holding the audio device into the next run. `kill_worker_group` derives the pgid and group-kills only on `live && parses && pgid>1 && pgid==pid && pgid!=driver pgid` — because a plain `cmd &` child reports the *invoking shell's* pgid, so an assumed pgid kills the driver and its wrapper. `reap_stray_players` then takes the players no group kill can reach (`--player-setsid on`, and `--die-after popen` where the leader is already gone) from the trace's `P_popen` pids, which is the only complete list, gated on a live-argv match against `$SID` so a recycled pid is not signalled. Sourced at the **top** of both drivers, before any worker exists, so an unreadable `cleanup.sh` fails while there is still nothing to leak — the round-30 draft sourced it at the trap, thirty lines after the worker launch, which is defect 5 re-armed by its own repair. Carries a per-site verdict for every other termination site in the rig, the ones that were already right included, and states its three residuals rather than claiming the cleanup is total. **Round 33: `reap_stray_players` reported the signal it SENT, not the process it REAPED** — harness defect 8, and harness defect **1**'s condition (`killpg` returned success, `nohup` had set the signal to `SIG_IGN`, nothing died, a false 12/12 was produced) arriving in the cleanup written to prevent exactly that class of leak. `kill(2)` only queues a signal: `player_probe.py` `fsync`s a `player_end` record before restoring `SIG_DFL` and re-raising, so it is alive when `kill` returns in **20 of 20** measured trials (0.50–0.99 ms). The signalled pids are now tracked and polled **as a set** for a bounded 2000 ms — `wait` is unavailable, these are the worker's children, and there is no `timeout(1)` on this macOS — survivors are escalated to **SIGKILL** and polled 1000 ms more, and only a pid alive after that is reported, as a `LEAK` block. A **zombie counts as gone** (it has exited and holds no device), so liveness is `kill -0` *and* a non-`Z` `ps` state. The leak is reported rather than returned as a driver status because it cannot be: bash takes the script status from the last command *before* an `EXIT` trap, so the only way to move it is an `exit` inside the trap, which would overwrite the driver's real verdict. Demonstrated three ways — stubs that honour SIGTERM reaped with none alive on return; a stub with `SIGTERM` set to `SIG_IGN` escalated and reaped, where the old code printed *"1 stray player(s) signalled"* and left it running; and an already-dead set entering no wait |
...
| `lock_overlap.sh` | **round 31: row 21's concurrency, which the owner count never established.** `run_lock.sh` counts `owner` records and the document reads a count of 2 as *"two resident workers"* — a claim about two processes owning the session **at the same time**. Nothing recorded a release and nothing compared two owners' intervals. This builds each owner an interval `[claim, release]` — `claim` being `mkdir_ok` / `published` and **not** `owner`, because a winner that has returned from `mkdir(2)` owns the lock while `owner` is where it *finishes publishing* one injected stall later — and reports the overlap. **4 of the 4 committed two-owner traces establish concurrency**, one of them (`lock-S2_longstall-spec-N4-s1000-r1`) with the racer's entire 500 ms hold **nested inside** the winner's interval although the two `owner` records are 0.939 s apart. It is a guard too: exits 2 on a two-owner trace that does not overlap, on an `owner` with no claim record, and on being handed no traces. `HOLD` is read out of `run_lock.sh` rather than typed in here, and a recorded `released` is preferred over `owner + HOLD` wherever a run wrote one. Reaches the **7 of 1200** trials that have a trace; the other 178 failing trials are inferred, and the document says so. **Round 33 closed the two ways it could pass without checking anything** (derivation defect 19, and the first time that shape has appeared twice in one file): the expected-trace-count check **defaulted to off**, so the invocation below never ran it and a deleted trace still exited 0 under a document quoting 7/1200 — the committed set's size is now the default and a non-numeric count is refused; and a run in which **no trace had two owners** printed *"nothing was checked here"* and exited **0**, so the guard establishing concurrent tenure could report establishment over an empty qualifying set — that is exit 2 now, as is a set of files that parse to no records. No figure moves; the committed set still reads 7/4/4. **Round 34 found a THIRD hole in the same guard, one round after the second was closed** (derivation defect 20): round 33's required trace count is over the **shell glob**, while the derivation dispatches per file on `FNR == 1`, which a **zero-byte** file never fires. So seven traces of which three are empty satisfy the count, report `traces read=4`, and exit 0 — the coverage fraction derived over fewer traces than its numerator names, which is what round 33 said it had fixed; round 33's `files == 0` guard catches only the case where *all* of them are empty, which is the case round 33 tested. Closed at both layers: the shell refuses a non-regular, unreadable or empty trace through `require.sh`, and `awk` refuses to report unless `files` equals the number of files it was handed, so a future dispatch condition subtler than emptiness cannot drop one silently either. Still 7/4/4. `lock_overlap.sh traces` |
| `tally_check.sh` | **round 32: the decision document's own tallies, recounted from their own row blocks.** Round 27 found the §6 induction count carried by hand in three disagreeing places; round 31 recounted it and wrote that *"every site now carries the same number"* while the paragraph **directly under the table** still read *"Fourteen rows"* over sixteen. A recount performed once expires the next time a row is added, so this is a script and a **guard**. It recounts the induction table's rows, both §1 defect lists (with a contiguity check) and §6's inline clause markers from the blocks they are counts of, then checks **sixteen** stated sites — the site count itself being recounted off the checks rather than restated. It exits 2 on a mismatch **and on a site it can no longer find**, because a tally guard whose pattern silently matches nothing reports a clean tally for a document that states none. **Round 34 widened three capture classes from `[A-Za-z]+` to `[A-Za-z-]+`**: `w2n` has understood `twenty-one` since round 32, but the patterns stopped at the hyphen, so the first time the derivation list crossed twenty all three sites reported MISSING SITE. That is the guard behaving correctly and refusing rather than passing — and the class was still too narrow for a number the document was always going to reach. It accepts more **words**, not more **values**. `tally_check.sh ../preemption-and-lock-protocol.md` |
