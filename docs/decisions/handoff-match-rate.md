...
120 s (`rewrite.sh:72`) **[repo]** and `hooks/hooks.json:9` declares a 120 s timeout for the display
hook **[repo]** — both were 45 and 60 when this was written, raised together for #14, which also
means the 52.50 s rewrite below now lands *inside* the budget rather than being TERMed by it — and the
one end-to-end rewrite anybody has timed took
