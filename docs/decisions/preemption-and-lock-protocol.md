...
- **Confirmed.** The adversarial ordering `R < S2 < R_b < P < W` was provoked and held on
  **48 of 48** trials, with the hook's own kill hitting nothing every time. Clause (iii), the
  worker's claim-time kill, is **load-bearing**: with it the stale player is killed **before it
  `exec`s** — it never logs a start and has no exit status — 36/36; without it, in the identical
  ordering, it runs to completion (12/12), and in real audio the two `afplay` **processes
  co-existed for 3.25–3.50 s** **[measured-here]**. **All three of those are process
  observations and this document no longer states them as audio ones.** Nothing here listened,
  `afplay`'s time-to-first-sample was never measured, and *whether* the stale utterance reached
  the output device — for any part of a 19 ms process life or of a 3.5 s co-existence — is
  **[inferred]** (§2.6). The 36/36 is the strongest of the three precisely because it is not an
  audio claim: a process killed before `exec` had not begun to open the device.
...
**Both rates count trials that did not produce exactly one owner. Round 31 asked whether that is
the same thing as two workers resident AT ONCE, and until round 31 nothing in the rig checked
it.** It does not follow from the count: `owners` counts ownership *conclusions* and no release was
recorded. It is now derived — `preemption-lock-probe/lock_overlap.sh` builds each owner's interval
from its **claim** (`mkdir_ok`, not `owner`) to its release, and **4 of the 4** two-owner trials
that have a per-trial trace hold the session concurrently, one of them with the racer's entire hold
**nested inside** the winner's **[measured-here]**. **No figure in the table above moves.** The
review finding that prompted it — that `spec`'s 1000 ms cell is sequential and both rates are
...
why it was derived rather than argued (§1, derivation defect 17).
...
thirteen"* under a table that had grown to fifteen rows while this sentence tracked it
correctly; the number is the row count and nothing else. **Round 31's recount then missed the
site directly under its own table** — *"Fourteen rows"* over sixteen, two rounds behind, while
claiming every site had been reconciled — so the recount is a committed guard now rather than a
past action: `preemption-lock-probe/tally_check.sh`, sixteen sites, failing on a mismatch and on
a site it cannot find (§1 derivation defect 18).
**Round 32's seven review comments were all already repaired** — every one of them was written
against an ancestor commit of this revision, and each is named in §6's table with the commit that
answered it. No figure moves for any of them, and **round 32's row is therefore a self-review row**
like rev 2's: the defect it records is in round 31's own completeness claim, not in anything the
reviewer raised. *"Each found a real defect in the previous round's confident answer"* is exactly
true of it and *"review found it"* is not, which is the distinction §6's row-per-round rule already
carries and this sentence should not blur.
...
the trial column; and round 31's in **row 21's metric itself** — a count of ownership
*conclusions* read throughout as a count of *concurrent holds*, with the concurrency never
checked, which makes it the first entry in that table where the reviewer's own inference was
also wrong and the underlying criticism still landed. See §6.
**Round 33's three findings are all against the two scripts the previous two commits added, and
two of them are one defect twice in one file: a guard that passes when it verified nothing.**
`lock_overlap.sh` — the derivation that makes §3.1's `owners` metric *derived* rather than
asserted — had a coverage check that **defaulted to off** in the only invocation this document
gives, and reported a run in which **no trace had two owners** as a successful derivation. Both
now refuse; no figure moves, and the committed set still reads 7 traces, 4 two-owner, 4 proven.
The third is harness defect 8, whose premise held (`player_probe.py` is alive when `kill` returns
in 20 of 20 trials) and whose stated mechanism did not (no wrapper in this rig continues past a
driver failure), leaving harness defect **1**'s shape — a signal sent, success reported, nothing
verified — as the actual defect. **What this round adds beyond three repairs is a name for the
...
explicit that it is insufficient, and the data shows why twice over: `C3` and `C6` have
**identical outcomes at the player** (both killed before `exec`, 12/12) and use different kill
targets; `C3` and `C10b` have **different** ones (killed before `exec` versus a 0.70 s process
life) while both are killed by the same clause. An outcome-only record would have merged the
first pair and split the second. **These are process outcomes and round 27 stopped calling them
audible ones** — the arms named here are stubs, in which no audio device is opened at all, so
*"audibly identical"* and *"both silent"* described a sound that never existed either way. The
paragraph's point survives the correction and is sharper for it: distinct signals are what make
the trace say which step fired, and no audio observation could.
...
wrote that every site now agreed. **Round 29's response to number 7 was therefore not to fix the two
...
**Number 19 is the fourth, and with it the shape stops being an observation about individual
entries and becomes a property of this list: a defect in a repair.** It is a defect in number
17's own repair script, found **one commit** after that script was written, and harness defect 8 —
found in the same round — is a defect in harness defect 7's repair, which was itself a defect in
harness defect 5's. So this round found two, in two different chains, and the general reading is
...
14. **AUDIBILITY WAS NEVER MEASURED AND WAS PUBLISHED AS `[measured-here]` ANYWAY — round 27,
   ...
   **process** stamps: `timerstart_to_exit_s` for the millisecond bound, and
   ...
   3.25–3.50 s one. **Nothing here listens, `afplay`'s time-to-first-sample was never
   instrumented, and a newer `afplay` process existing does not establish when its output became
   audible.** §2.6 had already conceded exactly this, for the millisecond figure only, in the
   paragraph immediately below the table; the concession never reached the overlap figure or any
   of the places either number is quoted. **That is the failure mode: one claim, corrected in one
   place.** All thirteen sites, now restated as process observations with audibility left
   `[inferred]`:
   1. §0's headline bullet — *"dies before it makes a sound"* **and** *"two utterances overlap
      for 3.25–3.50 s"*, under one **[measured-here]** tag.
   2. §1's *"Attribution is by distinct signal"* paragraph — `C3`/`C6` *"audibly identical (both
      silent)"* and `C3`/`C10b` *"audibly different"*, **in stub arms that open no audio device
      at all**, inside the paragraph whose own point is that audibility was never the
      observation.
   3. §1's statement of what `summarise.sh` covers — *"the audibility intervals"*. No TSV column
      holds one.
   4. §2.2's attribution table — the whole last column was headed *"stale audio, s"*, on **26
      rows of stub arms** whose player is a `sleep`.
   5. §2.3's clause (iii) bullet — *"two utterances overlapped"*.
   6. §2.6's opening sentence — *"this arm answers audibility only"*.
   7. §2.6's results table — *"overlapped for 3.2486, 3.2564 and 3.4981 s"*.
   8. §4d's proposed §13 row 20 — *"36/36 killed before any sound"* and *"two real utterances
      overlap"*, which is the wording that would have shipped into the spec.
   9. §5's limitation bullet — *"the real arm only answers audibility"*, closing on `n = 12`,
      which buys process samples and no audio observation.
   10. `run_real.sh`'s header — *"whether the stale utterance is audible, and for how long"*.
   11. `collect_real.sh`'s column comment — `overlap_s` as *"how long the two players were alive
       at the same time"*, with the *"neither is time audible"* caveat attached to the other two
       columns and not to this one.
   12. `summarise.sh` section E's preamble — same shape: it says *"neither is a measure of
       AUDIBILITY"* about the two lifetime columns and prints `overlap_s` beside them unqualified.
   13. `README.md`'s stub section — the real arm *"exists to answer the one thing the stub
       cannot: whether the stale utterance is audible"*.

   **The figures do not move and none is withdrawn.** They are re-labelled: process lifetime and
...
17. **ROW 21'S METRIC WAS READ AS A CONCURRENCY CLAIM AND NEVER CHECKED AS ONE — round 31, and it
   is the metric the whole of §3 turns on.** `run_lock.sh` counts `owner` records; §3.1 glosses a
   count of 2 as *"two resident workers, each holding a ~340 MB model"*, which asserts that two
   processes held the session **at the same time**. Nothing recorded a release and nothing compared
   two owners' intervals, so the reading was an interpretation sitting on top of the metric — the
   same shape as **defect 14** (a quantity called *audibility*, derived entirely from process
   stamps) and **defect 4** (a lifetime published as an interval it was not): the field is computed
   correctly and the gloss claims more than it derives. **Unlike 14 and like 4 it is fixable by
   re-deriving, and it has been:** `lock_overlap.sh` builds each owner's interval as
   `[claim, release]` — `claim` being `mkdir_ok` / `published`, **not** `owner` — and reports the
   overlap; **4 of 4** two-owner traces establish concurrent ownership. **The review finding that
   prompted this went further and does not survive**: it read the winner's `owner` record as the
   ...
   **182** split by shape into **140** with the stalled winner as a party and **42** that are two
   ...
   no sample is synthetic, and the correction adds a derivation rather than repairing a producer.
18. **A RECOUNT THAT CLAIMED EVERY SITE AND REACHED THREE OF FOUR — round 32, and the tally it
   got wrong is §6's own induction count.** Round 27 found this tally carried by hand in three
   places, all three disagreeing, *"in the section whose entire argument is an induction over that
   tally"*. Round 31 recounted it `awk`-over-the-row-block and wrote that **every site now carries
   the same number for the same reason**. The paragraph **directly under the table** still read
   *"Fourteen rows"* over a table with **sixteen** — the count as it stood two rounds earlier,
   which is exactly the drift rate the bullet above it measures, in the bullet asserting the drift
   had been stopped. **It belongs on this list and not the harness list, on the test entry 17
   defended**: the harness produced nothing wrong, no trial, sample or TSV is touched, and the
   quantity is computed correctly by an `awk` that simply was not pointed at one of its consumers.
   It is §1's own definition of the kind — *a derivation that does not reach the figure it is
   cited for* — with the shortest possible distance between the derivation and the figure it
   missed: **one paragraph.**
   - **THE REPAIR IS THE ONE ROUND 29 ESTABLISHED FOR A RULE WITH COPIES: give it one definition.**
     A recount performed once expires the next time a row is added, so the recount is now
     `preemption-lock-probe/tally_check.sh` — over the document itself, recounting the induction
     table's rows, both of this section's defect lists (with a contiguity check, since a list
     numbered 1..16,18 has seventeen items and a missing sixteenth) and §6's inline clause markers,
     each **from the block it is a count of**, then checking every stated site against its own
     recount. **Sixteen sites**, and the guard fails on a mismatch **or on a site it can no longer
     find**: a tally guard whose pattern silently matches nothing would report a clean tally for a
     document that no longer states one, which is the null-as-pass shape §1 keeps meeting (defects
     6, 10 and 16 and harness defect 6 are all a check that passed on nothing or on part of its
     input) -- and the count of those is deliberately not restated here, since restating a count is
     the defect this entry is about. **Both refusal modes are demonstrated in one run**, against the document as round 31
     committed it: the tally sites give fifteen `ok` lines and one `MISMATCH` — the site under the
     table — and the six site-count checks all report `MISSING SITE`, rev 14 having no such guard
     for its prose to state a count for. Exit 2 on both grounds; against this revision, exit 0
     with every site `ok` **[measured-here]**.
     **And the site count is recounted too**, off the checks themselves rather
     than restated, because a guard that cures a hand-maintained count by introducing its own
     hand-maintained count of sites is the same defect one level up.
   - **What it does NOT do.** It checks that each site agrees with the recount, not that the
     recount is the *right* thing to count — the row-per-round rule is still a judgement, and §6
     still says so. And the site list is anchored on sentence text, so rewording a checked sentence
     makes the guard fail loudly rather than drift silently; that is the intended direction, and
     re-anchoring is a deliberate edit rather than a maintenance cost avoided.
19. **ROW 21'S DERIVATION COULD PASS WITHOUT DERIVING ANYTHING — TWICE, IN ONE FILE, ONE COMMIT
   OLD — round 33.** `lock_overlap.sh` is entry 17's repair: it is the reason §3.1's `owners`
   metric is described as **derived** rather than asserted, and §3.3a quotes its coverage as
   **7 of 1200**. It had **two** null-as-pass paths, and they are the same path twice:
   - **The coverage check defaulted to off.** The expected trace count was the optional second
     argument under a `> 0` test, and the invocation this document and the rig's README both
     give is `lock_overlap.sh traces` — no second argument. So the check never ran in the only
     form anyone calls it in, and **deleting one of the seven committed traces left the script
     exiting 0 over a sample of six while the document went on quoting 7/1200**. The size of the
     committed set is now the default, a non-numeric count is refused rather than evaluated as
     zero, and `0` still disables the check but says so on stderr and is never reached by
     default.
   - **No qualifying trial was reported as a successful derivation.** Concurrency can only be
     tested on a trace with two or more owners. Where none had two, the script printed
     *"no traced trial produced two owners, so nothing was checked here"* — and **exited 0**. The
     one script cited as establishing concurrent tenure could therefore report establishment
     from a run that established nothing at all. Zero qualifying trials is now exit 2; so is a
     set of files that parse to no records, which the empty-glob guard above it did not catch.

   **No committed figure moves**, and that is checked rather than assumed: over the committed
   traces with no argument the script still reads **7 traces, 4 two-owner, concurrency proven on
   4**, and exits 0. **It is filed here and not on the harness list on the test entries 17 and 18
   were defended on** — no trial is mis-staged, no sample is synthetic, no TSV or trace is
   touched, and the arithmetic over the committed set was right both before and after. What was
   wrong is the sentence around it: a guard that *prints* a coverage fraction it does not
   *enforce*, and a derivation that reports success over an empty qualifying set, is §1's own
   definition of the kind — **a derivation that does not reach the figure it is cited for** —
   with the figure in question being the establishment claim itself.
   - **AND THE CLASS IS NOW NAMED, because this is the fourth member of it and the second in
     two rounds: a defect in a repair.** Entry 18 is a defect in entry 17's-round recount; both
     of the recurrences the note above describes (7 and 8) are a fix applied to one site and not
     its sibling; and this entry is a defect in **entry 17's own repair script, found one commit
     after it was written**. That is no longer a coincidence to be disclosed one instance at a
     time. **The repairs in this document fail at the same rate as the things they repair**, and
     the reading that follows is not that any one entry is unfinished but that *a fix is evidence
     of nothing until it has itself been reviewed* — which is the argument §6 makes about
     inspection, turned on this document's own patches. It is also why both of this entry's
     refusals are **demonstrated** rather than asserted: a trace directory with one trace
...
The fourth was found in round 3 and it touches a published figure; the fifth, in round 27, the
sixth, in round 29, the seventh, in round 30, and the eighth, in round 33, could only have
contaminated a *future* run rather than a past one — and the sixth is the only one on this list
that would have produced
**affirmative evidence for a mechanism out of a failure to measure it**, which is why it is the
sharpest of the eight despite moving nothing. **The seventh is the one that came out of another
entry on this list**: it is the incompleteness of the fifth's own repair, and it made the mistake
§4b clause 7(iv) exists to prevent — killing a worker's pid does not reach that worker's player —
in the rig built to measure that clause. **The eighth continues that same chain to a third link**:
it is the incompleteness of the *seventh's* repair — a leak check that reported the signal it sent
rather than the process it reaped, which is defect **1**'s condition (a signal delivered, success
returned, nothing dead) reappearing in the cleanup built to prevent it.
...
5. **A failed run left its worker running for ten minutes — round 27, and in the real-audio arm
   that stranded a loaded model into the retry.** `run_real.sh` and `run_preempt.sh` start their
   worker in the background, capture `WPID`, and then `exit 1` on a readiness timeout **without
   terminating it**: `run_real.sh`'s initial gate, and `run_preempt.sh`'s initial gate, its
   per-trial generation gate and its replacement gate — **four fatal paths across two drivers**.
   Every one of those workers carries `--idle-exit-s 600`, so each failed run leaves a live
   worker holding that configuration's speak dir, its election and its own generation for ten
   minutes. **In the real arm the stranded process also holds a ~340 MB Kokoro model**, which is
   how a harness leak becomes a measurement problem: the obvious response to a failed run is to
   run it again, and the retry's resource usage is then two workers while looking like one.
   Reproduced on the committed drivers before the fix: **the readiness gate strands the worker on
   both, and `C8_orphan`'s generation gate strands the RESTARTED worker while the first is
   correctly reaped** **[measured-here]**. Both drivers now install an **EXIT trap immediately
   after `WPID` is captured**, reading `$WPID` at trap time so it covers whichever worker
   `start_worker` last launched, and `wait`ing rather than only signalling; the per-site `kill`s
   are removed as redundant. **That trap was itself incomplete, and defect 7 is what it missed:**
   it signalled the worker's pid, which does not reach the worker's players, so this repair fixed
   the stranded *worker* and left a stranded *player* holding the audio device. Read the rest of
   this item as the worker half of a fix completed in defect 7.
   Demonstrated in both directions on each driver: readiness withheld →
   `rc=1` and the worker reaped; readiness granted → the gate passes and a later fatal path still
   reaps; and the whole happy path → `rc=0`, run directory printed, every worker reaped.
   **No committed figure is affected** — a run that took one of these paths produced no trace to
   collect, so nothing published came from one — and the defect is disclosed because the
...
6. **A REFUSED PUBLICATION WAS COLLECTED AS A SUCCESSFUL HOOK PREEMPTION — round 29. This one
   ...
   *"a record that certainly existed"* argument on any `W_pid_write`, refusals included — and now
   skips them.
7. **THE CLEANUP PATH MADE THIS DOCUMENT'S OWN CENTRAL MISTAKE — round 30, and it arrived through
   defect 5's own fix.** Defect 5's repair installs an `EXIT` trap in both drivers; the trap did
   `kill -TERM "$WPID"` **and nothing else**. But `speakd_probe.py` calls `os.setsid()` before it
   elects, so the worker **leads its own process group** and every player it forks is in *that*
   group — not the driver's, and not anything a signal to the worker's pid reaches. So a fatal
   readiness or hook path terminated the worker and left its player running: in `run_real.sh` an
   `afplay` decoding a real 5.7 s wav, **holding the audio device into the retry the operator
   starts next**; in `run_preempt.sh` a 2.5 s stub, and under `C17_setsid_player` a player that
   calls `setsid()` *itself*, so it sits outside even the worker's group and outside anything the
   trap could name.

   **§4b clause 7(iv) is the process-group sweep, and it exists precisely because killing a
   worker's pid does not reach that worker's player.** The rig made that error one function away
   from the code that measures it, and `C17` — the arm whose entire purpose is to show clause
   7(iv) failing — was the case it handled worst. It gets **its own number rather than being filed
   as a recurrence of 5**, on the test this list already applies: defect 5's consequence is a
   stranded **worker**, competing for the next run's jobs and resources; this one strands a
   **player**, which holds the audio device — *the single invariant this document exists to
   certify*. Same trap, different consequence, so the same rule this document used to give defect
   6 its own number applies. It is **one** defect and not two despite touching two drivers: one
   rule with two copies is the shape round 29 met in `attrib.sh`, and the repair is the same
   structural one — the rule, its guards and its residuals now have exactly one definition, in
   `preemption-lock-probe/cleanup.sh`, sourced by both drivers.

   **The over-kill direction is the dangerous one, and the guard is measured rather than
   reasoned.** `os.setsid()` makes the worker's pgid equal its pid — *but only from the instant it
   reaches that call*. Measured on this machine: a plain `cmd &` child reports the **invoking
   shell's** pgid, not its own pid. So `kill -TERM -<pgid>` against a worker that died in argparse,
   or that has not yet been scheduled, names the **driver's own process group** — killing the
   driver, the wrapper that invoked it (`run_all_preempt.sh` and its siblings run the driver in a
   command substitution) and the shell above that. `pgid_of()` therefore returns *nothing* for a
   dead pid, and the group kill fires on one conjunction: live, pgid parses, `pgid > 1`,
   `pgid == pid` — which **is** the verification that `os.setsid()` ran, since only a group
   leader's pgid is its own pid — and `pgid !=` the driver's own pgid. Everything else falls back
   to the plain pid kill, which at worst is `ESRCH`. **Demonstrated in both directions**: a fatal
   path with the worker leading its group reaps worker and both players and the driver survives;
   the happy path completes, prints its run directory, and the driver, its wrapper and the harness
   are all still alive afterwards **[measured-here]**.

   The two populations no group kill can reach are both staged deliberately — `--player-setsid on`
   (`C17`) and `--die-after popen`, which leaves the group with no live leader for the guard to
   verify — and both are reaped from the trace's `P_popen` pids, gated on a live-`argv` match
   against `$SID` so a recycled pid running a stranger's program is not signalled. The trace and
   not the player records, because a per-player record appears only after the wrapper's `mv` and
   the arms that matter stage the worker's death before it: `P_popen` is `fsync`ed the instant
   `Popen` returns, for every `--pid-mode`, so it is the only **complete** list. **`C17` is
   therefore cleaned up** — what it defeats is the process-group *mechanism*, not this cleanup,
   which does not rely on the group.

   **Three residuals are stated rather than implied.** A player both outside the worker's group
   *and* absent from the trace is reachable by neither mechanism — one window, between `Popen`
   returning and `P_popen`'s `fsync` completing under `--player-setsid on`, staged by no
   configuration (`--die-after popen` fires *after* `P_popen` is written) and not closable from
   the driver. A pid recycled between the `ps` and the `kill` is signalled as a stranger — one
   `kill(2)` wide, unavoidable without a pidfd. And the `shared`/`perplayer` wrapper forks
   `/bin/sleep` for its publish delay, so a kill inside that window orphans a `sleep` bounded by
   `--publish-delay-ms`, holding no device.

   **No committed figure is affected**, for the same reason as defect 5: a run that took one of
   these paths produced no trace to collect. The disclosure is here because the contamination it
   enables — a retry that begins with the previous run's player still holding the output device —
   is invisible in the run that suffers it, and because `cleanup.sh` now carries a per-site verdict
   for **every** termination site in the rig, the already-correct ones included. Two of those are
   worth naming: `run_preempt.sh`'s **per-trial** kill is a pid kill *on purpose* and must not be
   "fixed", because it is staging rather than cleanup — the previous generation's orphaned player
   is the subject of the trial, and a group kill would destroy what the trial measures; and
   `speakd_probe.py`'s `idle_exit` path exits without signalling a player still in its group, a
   real gap that no configuration reaches (`--idle-exit-s 600` exceeds every run, every player is
   bounded) and that is recorded rather than closed. The general rule, which is worth more than
   the two edits: **any place this rig terminates something, ask what that thing's children are
   in.**
8. **THE LEAK CHECK REPORTED WHAT IT SENT, NOT WHAT IT REAPED — round 30's repair, round 33, and
   it is harness defect 1's reporting shape arriving in the cleanup.** `reap_stray_players` — the
   half of defect 7's fix that reaches the players no process-group kill can name — ended with
   `kill -TERM "$pid"`, a count of those calls, and `return 0`. `kill(2)` only queues a signal, so
   **that count never established that one player had stopped.** Measured on this machine:
   `player_probe.py` is still alive when `kill(SIGTERM)` on it returns in **20 of 20** trials, for
   **0.50–0.99 ms**, because its handler writes and `fsync`s a `player_end` record before it
   restores `SIG_DFL` and re-raises **[measured-here]**. **Defect 1 is this document's own
   precedent for why sending is not reaping**: there, `nohup` had set the swept signal to
   `SIG_IGN`, `killpg` returned **success**, nothing died, and the run produced a false 12/12. A
   signaller that reports the signal as the outcome cannot tell that case from a clean reap — and
   this one is the leak check, so *"signalled"* being read as *"gone"* is the whole of its value.
   It gets its own number on the test this list already applies, and it is the **third link in one
   chain**: defect 5 stranded a worker, defect 7 was the incompleteness of 5's repair and stranded
   a player, and this is the incompleteness of 7's repair — a player reported as dealt with while
   still running.

   **The review finding that prompted this named a mechanism that does not exist, and the
   correction matters more than the fix.** It said the wrapper *"may start the next configuration
   immediately"*. It may not: **every** wrapper in the rig — `run_all_preempt.sh`, `run_c10.sh`,
   `run_c16.sh`, `run_tail.sh`, `run_missing.sh`, `run_pgid_rerun.sh` — `exit 2`s on a driver
   failure and never reaches the next configuration, which was verified rather than assumed. And
   on the path where a wrapper *does* continue — a successful run — there is nothing to signal:
   `run_preempt.sh` ends every trial with `sleep 6` against a 2.5 s stub and `run_real.sh` with
   `sleep 12` against a 5.7 s wav, so both drivers outlive their last player by seconds. The
   sub-millisecond window is therefore **not** a cross-run contamination path. What is left is the
   defect above, which does not need the window: **a player whose SIGTERM is ignored or that is
   wedged would be reported as `signalled` and left running indefinitely**, and nothing checked.

   **No committed figure could have been produced under it, and the ground is checkable rather
   than argued**: `reap_stray_players` exists only in `cleanup.sh`, added by `92d4586` in round
   30, and the committed traces were last written by `07c3944` — an **ancestor** of it. The
   function has never run in a run that produced published evidence. Its predecessor, the bare
   `kill -TERM "$WPID"`, is defect 7, already disclosed on the same footing.

   **The repair waits, escalates and reports two numbers instead of one.** The signalled pids are
   tracked and polled as a **set** — serialising the waits would make the budget `n × grace` and
   let an early wedged player hide a late one — for a bounded 2000 ms; survivors are escalated to
   **SIGKILL**, which no disposition can ignore, and polled for a further 1000 ms; only a pid
   alive after *that* is reported, as a `LEAK` block naming it, since there is nothing stronger
   left to escalate to. `wait` is unavailable (these are the worker's children, not the driver's)
   and there is no `timeout(1)` on this macOS, so the deadlines are explicit polls. **A zombie is
   counted as gone** — it has exited, holds no device and no CPU, and counting one would be a
   false leak report from an unreaped wait status — so liveness requires `kill -0` *and* a
   non-`Z` state from `ps`. **The leak is reported and not returned as a driver status,
   deliberately and because it cannot be**: bash takes a script's status from the last command
   *before* an `EXIT` trap, not from the trap — verified here, a trap returning 7 leaves `exit 0`
   at 0 and `exit 1` at 1 — and the only way to move it is an `exit` inside the trap, which would
   overwrite the driver's real verdict with the cleanup's opinion of a stray. **Demonstrated in
   three directions**: three `player_probe.py` stubs that honour SIGTERM are reaped with none
   alive on return; a stub with `SIGTERM` set to `SIG_IGN` — defect 1's exact condition, which
   the old code reported as `1 stray player(s) signalled` before leaving it running for another
   120 s — is escalated and reaped; and a trace whose players are already gone enters no wait
   **[measured-here]**. **One residual is unchanged and restated because this function now makes a
   claim**: the wait covers the pids it *signalled*, so the `/bin/sleep` that the
   `shared`/`perplayer` wrapper orphans inside its publish delay is still bounded by
   `--publish-delay-ms` rather than reaped, and is still not in the trace.
...
trial**, so `summarise.sh` covers per-trial quantities — the attribution table, the **player
lifetime and process-overlap** intervals (round 27: this list said *"the audibility intervals"*,
and no TSV column holds an audibility interval, because none was measured), the window widths,
the owner counts. Figures that are counts of trace **events**
...
`killpg`-used/skipped counts come only from `analyse_round2.sh`.
**Round 31 adds ANOTHER derivation on the far side of the same line, for the same reason the ones
already there are there: a figure no committed column can carry has to be somebody's job.**
`lock-owners.tsv` holds one
owner **count** per trial and no timestamps, so no section of `summarise.sh` can say whether a
trial's two owners held the session **at the same time** — which is what §3.1's *"two resident
workers"* asserts. `lock_overlap.sh preemption-lock-probe/traces` derives it from the per-trial
lock traces, and reaches the **7 of 1200** trials that have one (§3.4). It is a guard as well as a
derivation: it exits 2 on a two-owner trace whose intervals do not overlap, on an `owner` with no
claim record to build an interval from, and on being handed no traces at all — and, since round 33,
on being handed a **different number** of traces than the fraction above names, and on a run in
which **no trace qualified** to be checked at all. Both of those used to exit 0, which is
derivation defect 19: the fraction was printed and not enforced, and an empty qualifying set was
reported as a successful derivation.
Round 16 found `summarise.sh`'s
...
**The last column used to be headed *"stale audio, s"* and that was wrong on every row —
corrected in round 27 (§1 derivation defect 14).** Every configuration in this table is a
**stub** arm: the player is a controlled `sleep`, no audio device is opened and nothing is
synthesised, so there is no audio to have a duration. What the column holds is the stale
...
- **Clause (iii), the claim-time kill: REQUIRED, and confirmed by falsification.** Remove it
  ...
  it. In real audio the two `afplay` **processes co-existed for 3.25–3.50 s** — a
  process-overlap bound, not a measurement of overlapping sound; audibility is **[inferred]**
  and §2.6 says why.
...
distinguishable utterances, the `C3` ordering, 3 trials per arm. The stub arms establish the
ordering and the attribution; this is the only arm that used real synthesis and a real player.

**IT DOES NOT ANSWER AUDIBILITY, and round 27 removed the claim that it does.** This section
used to open *"this arm answers audibility only"*, and every figure in it is derived from
`afplay` **process** stamps — `Popen` return, reap — with **nothing listening at any point**.
This section held two of the sites; **seven further sites in this document and four in the rig**
then repeated the process numbers as audio ones. §1's derivation defect 14 enumerates all
thirteen. What the arm actually adds over a stub is that the process
being killed was a real `afplay` on a real 5.7 s wav rather than a `sleep`, which is worth
having and is not the same thing as hearing it.

| arm | result — **process observations** |
| --- | --- |
| claim-time kill **on** | the stale `afplay` process existed for **at most 13.0, 17.5 and 18.6 ms** against a 5.7 s wav; the newer utterance's process then ran to completion, 3/3 |
| claim-time kill **off** | both `afplay` processes ran to completion and **their lifetimes overlapped by at most 3.2486, 3.2564 and 3.4981 s**, 3/3 |
...
**AND THE 3.25–3.50 s FIGURE HAD NO SUCH PARAGRAPH UNTIL ROUND 27, WHICH IS THE DEFECT.** The
limit below was written for the millisecond figure and applied only to it, while the overlap
figure — the one this document quotes six times as *"two utterances overlapped"* — kept its
audio wording. It is derived the same way and has the same limit, plus two of its own.
`collect_real.sh` computes it as `EX[jNa] − POP[jNb]`: the **reap stamp of the stale player**
minus the **parent's `P_popen` stamp for the newer one**. Neither endpoint is an instant at
which anything was audible, and **both endpoints push the number up**, so the figure is an
**upper bound on process co-existence**:
- `POP[jNb]` is stamped in the worker the moment `subprocess.Popen` **returns**. This arm runs
  `--pid-mode worker`, so that `Popen` is `["/usr/bin/afplay", wav]` with no publish wrapper —
  the newer process therefore does exist at the interval's start, but it has not necessarily
  reached `exec`, and it certainly has not opened the output device or handed it a sample. **The
  interval starts strictly before any newer sound could begin.**
- `EX[jNa]` is the **reap** stamp, at or after the stale process's real exit — and `afplay`
  exiting is itself at or after the last sample it wrote, not when the device stopped playing.

So the honest sentence is *"the two `afplay` processes co-existed for at most 3.25–3.50 s"*, and
any statement about overlapping **sound** is a further inference on top of that — one this
harness cannot make, in either direction, because it never observed the device.

**One honest limit, and the two corrections above do not touch it — they widen its scope.** What
is bounded is process existence, on both arms. Whether ≤ 19 ms is short enough that no sample
reached the output device is **[inferred]**, and so is whether any part of the ≤ 3.50 s
co-existence was two voices at once: `afplay`'s time-to-first-sample was not measured, and
nothing here listens. If anything the corrections sharpen the limit rather than weakening it:
`popen_to_exit_s` shows the process was reaped 1.6–2.8 ms after `Popen` returned, which makes
the audibility question turn almost entirely on launch latency the harness never instrumented.
**The experiment that removes the inference is a loopback capture, not a longer run** — record
the output device through an aggregate/loopback sink for the duration of each trial and derive
first-sample and two-voices-at-once from the recording. Until that exists, **no audibility
figure in this document is `[measured]`, and none is written as one**; `n = 12` per arm (§5)
would buy more process samples and not one audio observation.
...
**That second sentence is an interpretation of the metric and not the metric, and until round 31
nothing checked it.** The count is of `owner` **records** — ownership *conclusions* — and "two
resident workers" is a claim that two of them held the session **at the same time**. Nothing in
`run_lock.sh` records a release or compares two owners' intervals, so review was right that the
concurrency was asserted. **§3.4 derives it** from the committed per-trial traces, and states
what the untraced trials rest on instead. **No count and no rate in §3.2 changes**; what changed
is that the reading above is now derived where the evidence reaches and labelled where it does
not.
...
**Read the two rates as what the column says — trials that did not produce exactly one owner —
and not as a count of trials in which two workers were resident at once.** Those are the same
...
unconfirmed. **"Self-proving" is a statement about STAGING and not about concurrency**, and round
31 found that the two had been running together: a mis-staged trial yields 1 owner, so a 2-owner
trial proves its own staging — and proves nothing about whether the two owners held the session at
once, which §3.4 derives separately.

### 3.4 Does `owners = 2` mean two owners AT ONCE? — round 31, and the answer is yes where the evidence reaches

**Round 31's finding is that this document never checked its own central metric.** `owners` counts
`owner` **records** — ownership *conclusions* — and §3.1 glosses 2 as *"two resident workers, each
holding a ~340 MB model"*, which is a statement about two processes holding the session **at the
same time**. Nothing in `run_lock.sh` recorded a release, and nothing compared two owners'
intervals. **The criticism is correct and the repair is below.** What the finding also claimed —
that 20 of `current`'s 121 and 20 of `spec`'s 61 are therefore not concurrency violations, so both
rates are inflated — **does not hold, and the reason is a fact about the rig rather than an
argument.**

**The arithmetic it rests on is exact.** In `traces/lock-S2_longstall-spec-N4-s1000-r1.tsv` the two
`owner` records are **0.938533 s** apart while `HOLD` is **500 ms** (`run_lock.sh:45`), so the
racer's hold ended roughly 438 ms before the winner's `owner` record. **The inference from it is
wrong, in one specific place: the winner does not claim at its `owner` record.**

| the winner's own timeline, from that trace | t | what it is |
| --- | --- | --- |
| `mkdir_ok` | 1787685552.371967 | **the successful exclusive create — the election is decided here** |
| racer's `owner` | 1787685552.449920 | 78 ms later, inside the winner's window |
| `pid_written` | 1787685553.387963 | the 1000 ms stall has expired; publication completes |
| `owner` | 1787685553.388453 | the winner *finishes publishing* what it won 1.016 s earlier |

**A winner that has returned from `mkdir(2)` owns the lock.** The pid write is publication of an
already-decided fact, which is the entire premise of S1 and S2: §3.0 says row 21 is about
misclassifying the winner *"after the election has been decided"*, and there would be nothing to
misclassify if the winner did not already own it. So the winner's ownership interval is
**[`mkdir_ok`, release]**, and in that trace the racer's whole 500 ms hold sits **inside** it. The
two owners are not sequential; one is **nested** in the other. Read the same way, the 0.939 s gap
is not evidence against concurrency — it is the width of the misclassification.

**`preemption-lock-probe/lock_overlap.sh` derives this rather than arguing it**, over the committed
per-trial lock traces, and it is a guard: it exits 2 if any two-owner trace fails to establish
concurrent ownership, if an `owner` has no claim record to build an interval from, or if it is
handed no traces at all. An interval is `[claim, release]` where `claim` is `mkdir_ok` /
`published` and `release` is the trace's own record where a run wrote one, else `owner + HOLD`
with `HOLD` read out of `run_lock.sh` rather than typed into the checker. Over `traces`:

| trace | owners | owner-to-owner gap | interval overlap |
| --- | --- | --- | --- |
| `lock-S1_init-current-N2-s50-r1` | 2 | 0.046451 s | **0.500069 s, nested** |
| `lock-S2_longstall-spec-N4-s1000-r1` | 2 | **0.938533 s** | **0.500062 s, nested** |
| `lock-S3_aba-current-r1` | 2 | 0.117763 s | **0.382338 s** |
| `lock-S3_aba-spec-r1` | 2 | 0.107356 s | **0.392736 s** |
| the three 1-owner traces | 1 | — | not in question |

**4 of 4 [measured-here].** **No cell moves, no trial is withdrawn, and `current`'s 121/400 and
`spec`'s 61/400 stand exactly as printed.**

**THE DIRECTION OF THE ERROR IS WORTH SAYING OUT LOUD, because it is the opposite of almost every
other disclosure in this document.** Had the finding held, it would have removed trials from the
failure counts of `current` and `spec` — the two protocols being falsified — and left `proposed`'s
clean sheet untouched. It would have made **the replacement look relatively better than the
evidence supports**, where §0's other withdrawals (the pooled `P`→`W` figure, *"none confirmed"*
for reclamation, the audibility relabelling) all cut *against* the case being argued. A disclosure
that flatters the author's own proposal deserves more scepticism than one that costs him
something, not less, and that is why this one was derived to four decimal places instead of
reasoned about.

**What the derivation does NOT reach, stated as a fraction rather than as a reassurance.**
...
`released` record in every log. `lock_overlap.sh` over its 60 logs: **15** trials produced two or
more owners and **concurrency is proven on 15 of 15**, every interval end a **recorded** release
rather than an inferred one (79 recorded, 0 inferred). The direct answer to the finding is in the
widest gaps it printed — an `S2` cell at the 1000 ms stall where two `owner` records are
**1.000926 s** apart and the intervals still overlap by **0.508357 s**, nested, and a four-owner
`spec` trial in the same cell where **6 of 6** pairs overlap. Both `S3` trials — the *racer +
racer* shape, the one the ordering argument does **not** cover — are among the 15.
**None of that run's outcome counts is republished and none of it touches §3.2**: a fresh sweep on
a different machine state produced two owners in cells where the committed run produced one, which
is exactly why the committed numbers are the committed numbers. What it demonstrates is the
*property*, on 15 independent trials instead of 4, with the release observed rather than assumed.

**The closing condition, and half of it is already in the rig.** `lockrace.py` now records
`released` when a hold ends, so a re-run **observes** each ownership interval instead of inferring
its end from `hold_ms` — demonstrated in both directions, including a synthetic early release that
the `owner + HOLD` inference would have passed and the recorded value correctly refuses, and the
`--dead-owner` staging incumbent correctly recording no release because it never owned anything. The other
half is a producer change this round deliberately did **not** make: requiring the first owner to
hold until every participant has made its deciding claim would guarantee the overlap rather than
observe it, and manufacturing the answer in the rig is worse than the gap. **So the 178 close on
the same re-run §3.3 already requires**, with `lock_overlap.sh` run over its per-trial logs rather
than over 7 of them; §5 carries it. Filed as **derivation defect 17** — the numbers were right and
the *reading* of them was not, which is defect 4's and defect 14's kind and not the harness list's
(§1).
...
>   one `readlink`. **A loser of the PUBLICATION race retries.** `symlink` returning `EEXIST`
>   means only that someone else got to this generation number first, so the elector re-reads
>   the highest generation and starts over — `lockrace.py:370` and `:391` both
>   `rec("publish_lost")` and `continue`. **This is what gives S3, S4 and S6 their liveness**
...
>
>   **THE OLD CLAUSE ALSO SAID *"it does not fail"*, AND THAT WAS FALSE — corrected in round
>   27.** The retry loop has a bound, 60 attempts, in both implementations
>   (`lockrace.py:364`/`:393`, `speakd_probe.py`'s `ELECT_ATTEMPTS`), and falling off the end
>   returns the same `False` that losing to a live owner returns; in `speakd_probe.py` that is
>   `sys.exit(0)` and the worker does not start. So there are **three** terminal outcomes, not
>   two, and the clause has to name all three. (The sentence also cited the wrong two lines —
>   `:243`/`:249` are the `current` protocol's `unlink`/`rmdir` path, not `publish_lost`.)
>   - **WON** — this elector created the highest generation.
>   - **LOST** — a live owner was observed. Correct, terminal, and the design's intent.
>   - **EXHAUSTED** — the budget ran out with no owner observed either way. **This is a real
>     liveness failure and is specified rather than denied.**
>
>   **What spends the budget is two different things and only one of them is a spin.** A
>   collision is progress *by somebody else*: the next `highest_gen()` reads a higher
>   generation, so spending all 60 that way needs 60 successful publications inside one
>   election, and retrying with no pause is the right response. An **unreadable** record is not
>   progress at all — `highest_gen()` returns the same `g` and the read fails on it again — so
>   unpaced it burnt the whole budget in **microseconds** and gave up. Measured on the committed
>   probe as it stood: a `worker.lock.0` symlink whose target does not parse as a pid made the
>   worker **exit 0 after 0.12 s with `signal_dispositions` as the only line in its trace**
>   **[measured-here]**. A worker that declines to run and says nothing about why is the
>   null-as-pass shape of this rig, in the election — **the same shape round 15 found one
>   protocol over**, where `lockrace.py`'s `rmdir_failed` was the one exit from an election that
>   recorded no terminal outcome (`lockrace.py:248`). That one was found and named; this one was
>   in `speakd_probe.py` the whole time, and the clause above asserted it could not exist.
>   - **The exhaustion is now RECORDED, with the breakdown**: `election_gave_up attempts=
>     collisions= unreadable= lookup_failures=`. "60 collisions" and "60 unreadable reads" call
>     for opposite operator actions, and `election_lost` must not be able to stand in for
>     either. Also recorded per attempt is *why* a read failed — **absent** (removed between the
>     `readdir` and the `readlink`, genuinely transient) versus **unparseable** (present and
>     malformed, which no retry will fix).
>   - **The unreadable path gets a 20 ms backoff, so the budget spans a STATED interval** —
>     at most `60 × 20 ms = 1.2 s` — rather than an unstated one. Same input as above, after the
>     repair: **60 `owner_record_unreadable … why=unparseable` records over 1.2 s, then
>     `election_gave_up attempts=60 collisions=0 unreadable=60 lookup_failures=0`**, and no
>     `ready` and no `election_won` **[measured-here]**. The collision path deliberately keeps
>     no backoff.
>   - **The recovery policy: exhaustion costs ONE WORKER INVOCATION, not the session.** This
>     rests on a property of the caller and is stated as such: **PR #27 §10.5 clause 4's
>     ensure-worker step runs on *every* `MessageDisplay` invocation**, so a worker that
>     exhausted its budget is re-elected by the next invocation from a fresh one. Nothing
>     durable is written by a worker that exhausted, and no job is consumed. If that clause ever
>     becomes a once-per-session step, this policy is void and the exhaustion becomes a
>     session-level failure — which is why the dependency is named here rather than assumed.
>     **What it does NOT do is retry until a live owner is observed**, and that is the choice:
>     an unbounded retry against a permanently malformed record is a worker that never returns,
>     which converts a per-invocation liveness failure into a hung hook. The price is stated
>     rather than hidden — **a persistently unreadable record wedges the session**, and it
>     wedges it exactly as the `unverifiable`/pid-held refusal above does: liveness not safety,
>     one session (`$BUF_ROOT/<session_id>/speak/`, cleared by removing that directory), and
>     loud, now that the give-up is recorded. **[inferred]**, since no arm has exhausted a
>     budget in a measured run; the closing condition is an arm that seeds a malformed
>     generation record and counts the elections it costs.
...
>   after both halves of ITS OWN election sweep against `g`'s owner have reached a TERMINAL
>   result** — 7(iv) and 7(iv-a) — **and only while holding a generation above `g`**. Round 4
>   wrote the second condition as *"only the worker that created `g+1`"*; that is a strictly
>   stronger rule and it made cleanup non-recoverable, which is the round-22 paragraph at the end
>   of this bullet.
...
>
>   **AND *"only the worker that created `g+1`"* MADE CLEANUP NON-RECOVERABLE — round 27, and
>   this is a defect in the tightening, not in the ordering.** The two conditions were written as
>   one, and only the first is load-bearing. If the creator of `g+1` crashes **after** its
>   terminal sweeps and **before** its unlink — a window that includes the whole model load and
>   every job it serves — then no worker is ever again permitted to remove `g`. This document has
>   already established that nothing external reaches these records (`rewrite.sh:117` cannot
>   match a depth-3 symlink), so `g` is immortal, and **a crash loop accumulates generations
>   without bound**: every restart adds one, every `readdir`/`readlink` of the election and every
>   pass of the sweep pays for all of them, and the only thing that ever clears them is the
>   session directory going away.
>   - **THE PERMISSION, AND ITS CONDITION.** A worker whose own generation is `n` may unlink
>     generation `g` **iff (a) `g < n`** — it is obsolete, and `n` still exists, so nothing can
>     mistake the removal for reclaiming a live lock — **and (b) THIS worker's own election sweep
>     against `g`'s recorded owner has itself reached a terminal result in both halves**, with
>     `g`'s `.pending` marker retired in the same step.
>     **"Re-establish" has to mean re-perform, and that is the whole of the condition.** A later
>     worker may not inherit, assume or read off the crashed worker's sweep result — there is
>     nothing on disk that records one — so the permission is not an exception to the ordering,
>     it is the ordering with the identity of the sweeper dropped. The clause needed `g < n`
>     rather than `g = n − 1` for a reason that was already in the code: `sweep_pgid` walks
>     `for gg in range(g, -1, -1)`, so **an elected worker already sweeps every generation below
>     its own**, and it was only ever forbidden to *finish the job* on the ones it did not
>     directly supersede. The interleaving the rule exists to prevent is untouched: at the
>     instant of any unlink, that generation's owner group has been signalled or is provably
>     gone, so a later sweep's `continue` past a missing `g` loses nothing. What is still
>     forbidden — and this is the half that was doing the work — is unlinking on *completion*,
>     or on somebody else's terminal result.
>   - **WHAT IT COSTS, stated.** (1) **It does not eliminate the accumulation, it bounds it to
>     the non-terminal verdicts.** `recycled`, `unverifiable` and `lookup_failed` are deliberately
>     *not* terminal (4b(iv)), so a generation whose owner pid has been recycled is never
>     unlinked by anyone — correctly, since signalling it would hit a stranger's group. Those
>     generations still accumulate, but only for as long as the session lives, and each costs one
>     `readlink` and a recorded skip rather than a `killpg`. (2) **A generation's group can now be
>     signalled more than once**, once per elected worker that reaches a terminal `same` verdict
>     on it before somebody unlinks it. Harmless — the group is being killed either way, and a
>     repeat against a gone group is `ESRCH` — but it is a visible duplicate in the trace and
>     must not be read as two orphans. (3) **Concurrent unlinks of the same `g` are possible**
>     now that the unlinker is not unique: a worker at `n` and one at `n+1` can both qualify. The
>     loser gets `ENOENT`, which is not an error here, and a generation number is never reused,
>     so nothing can be recreated under a racing unlink. (4) The per-election work is
>     `O(generations still present)`, which is exactly what the permission exists to keep small.
>   - **Closing condition.** `[inferred]` and unrun, like the rest of this bullet. To close: add
>     the unlink, add a death point **between the terminal sweep and the unlink**, and show both
>     directions — `g` still present after the creator of `g+1` dies there, then unlinked by the
>     next elected worker once *its own* sweep of `g`'s owner terminates; and `g` **retained** by
>     that worker when its verdict on `g`'s owner is `recycled`.
>
...
> `exec`**) and its absence fatal (**12/12** run to completion; two real `afplay` **processes
> co-exist** for **≤ 3.25–3.50 s** — a process bound, audibility **[inferred]**, §2.6).
> **The two-kill partition leaves a third region** — a worker dying between
...
> rule §4a now states (unlink `g` only after both halves of the unlinking worker's **own**
> election sweep against `g`'s owner have reached a terminal result, and only while it holds a
> generation above `g`) has no arm behind it — **including the round-22 recovery permission**,
> whose closing arm is a worker killed between its terminal sweep and its unlink.
...
- **AUDIBILITY ITSELF — nothing in this document measured it, and until round 27 the
  real-audio arm was described as though it had.** Every figure in §2.6 is derived from
  `afplay` **process** stamps (`Popen` return, reap); **nothing listened at any point**, and
  `afplay`'s time-to-first-sample was never instrumented. So *"the stale player did not last
  ...
  **Nothing here can close this, and that is the
  ...
  substitute.** Which is the correction to the bullet this replaces: it said the real arm
  *"only answers audibility"* and named `n = 12 per arm` as the closing condition, and n = 12
  buys twelve more process samples and no audio observation at all. Running it is still worth
  doing for the process figures (~5 minutes, real synthesis is local and free), and it is a
  scope choice; **audibility is a missing instrument, not a missing sample size.**
- **WHETHER A FAILING TRIAL'S TWO OWNERS HELD THE SESSION AT ONCE — derived on 4 of the 182,
  inferred on 178, and round 31 is the first round to state it as a coverage fraction.** §3.1
  reads `owners = 2` as *"two resident workers"*; the metric counts ownership **conclusions**, and
  ...
  when a hold ends, so a re-run observes each interval instead of inferring its end from
  `hold_ms`. To close: **the S1–S6 re-run §3.3 already requires**, keeping every trial's log rather
  than 7 of them, then `lock_overlap.sh` over all 1200. **What was deliberately NOT done** is the
  other half — making the first owner hold until every participant has made its deciding claim.
  That would *guarantee* the overlap instead of measuring it, and a rig that manufactures its own
  headline is worse than a stated gap; it is named here so a later round does not read the
...
- **There are TWO garbage collections, neither is implemented or measured, and round 3 named
  ...
  - **Lock generations.** `lockrace.py` never unlinks an obsolete generation, so **all 400
    ...
    is a `continue` rather than a signal: **unlink `g` only after both halves of the unlinking
    worker's OWN election sweep against `g`'s owner have reached a terminal result, and only
    while it holds a generation above `g`.** Round 4 wrote that second condition as *"only the
    worker that created `g+1`"*, and **round 27 found that this made cleanup non-recoverable** —
    a creator that crashes between its terminal sweep and its unlink strands `g` forever, and a
    crash loop then accumulates generations without bound, since nothing external reaches a
    depth-3 symlink. §4a states the recovered permission, its condition and its four costs. All
    **[inferred]**. To close: add the unlink and re-run S3, S4 and S6 with a reclaim delay
    straddling it, add an arm that kills the electing worker between the unlink and the
    sweep, and add the mirror arm that kills it between the **sweep and the unlink** and shows
    the next elected worker completing the cleanup — and refusing to, on a `recycled` verdict.
...
argument about labels — it is an induction over every round in the table below.**

### What the rounds actually did

**THE COUNT IS THE TABLE'S OWN ROW COUNT, and round 27 made it so.** This heading read *"what
the nine rounds actually did"* over the **eleven** rows the table then had, while the paragraph
...
- **AND THIS PARAGRAPH IS WHY THE RULE HAD TO BE MADE MECHANICAL, because it broke again in the
  ...
  **AND ROUND 31's OWN RECOUNT WAS INCOMPLETE, WHICH IS DERIVATION DEFECT 18 AND THE THIRD TIME
  ...
  read *"Fourteen rows"* over a table that had sixteen — **two rounds behind, which is exactly the
  drift rate this paragraph measures**, in the bullet claiming the drift had been stopped. A
  recount performed once expires, and a claim to have recounted *every* site is the shape this
  document's derivation list is mostly made of: a repair applied to one site and not to its
  siblings (§1's recurrence note). **So the recount is now a committed guard rather than a past
  action**: `preemption-lock-probe/tally_check.sh` recounts the table's rows, both §1 defect
  lists and §6's clause markers from their own blocks, checks **sixteen** stated sites against
  them, and exits 2 on a mismatch **or on a site it can no longer find** — because a tally guard
  whose pattern silently matches nothing is the null-as-pass shape §1 keeps meeting.
  Executed against the document as round 31 committed it, it isolates
  the one wrong site out of sixteen and refuses **[measured-here]** — §1's entry 18 has the whole
  output, including the six site-count checks that report `MISSING SITE` on a revision with no
  such guard to describe. A tally maintained by hand is a tally with a drift
  rate, and this one's is now measured twice: **two rounds behind after three rounds**, and two
  rounds behind again one round later.
...
`awk` row count and a mismatch fails the guard. **Round 27 is the sharpest form of the pattern in the DOCUMENT**:
two of its four findings are defects in clauses this document itself proposed as repairs (round
4's cleanup ordering, round 21's election policy), and a third is in the document's own choice of
noun — a figure named *audibility* and derived from process stamps, sitting one paragraph above
the sentence that says audibility was never measured. Nothing in any script was wrong about it;
the words were. Rev 5 was the sharpest form of the pattern in the staging: **four of its five
...
1. **"Obviously correct on inspection" has now failed on seventeen distinct clauses.** Counted
   the way the list below reads: **the two lock clauses are two**, and **clause (ii)'s scope is
   one clause that failed twice** — the count is of clauses, not of failures. (1) and (2) the two
   lock clauses; (3) clause (ii)'s scope (twice: first mis-scoped, then credited to the wrong
   sweep); (4) the shared-record lifecycle; (5) the append-only ledger's truncate; (6) the
   player-authored-record repair; (7) the process-group sweep's own blast radius; (8) the sweep's
   dependence on an unstated `setsid()`; (9) the missing record sweep at election time; (10) the
   generation GC's non-existent reclaimer; (11) the `.pending` GC's undeterminable set; (12) the
   ...
   and, in round 21, (13) the **PLAYER record's** absent identity (three signalling sites taking a
   pid from a record that outlives its process, with an anchored *name* mistaken for an
   identity) and (14) the **election's** treatment of an unverifiable owner (superseding read as
   harmless because the sweep refuses separately, when superseding is the licence to consume
   jobs and therefore the route to two owners) — and, in round 26, (15) the **claim-time kill's
   ...
   `C10b` trace contradicts 12 times over — and, in round 27, (16) the **generation-unlink
   ORDERING rule**, whose *"only the worker that created `g+1`"* made cleanup non-recoverable and
   let a crash loop accumulate generations without bound (a different clause from (10), and a
   defect in the repair for it), and (17) the **ownership clause's liveness claim**, *"a loser
   retries; it does not fail"*, which contradicted a 60-attempt bound present in both
   implementations from the start.
   Inspection is what produced all seventeen. The player-authored-record repair is the sharpest
   ...
   **(17) is a third kind, and worth naming separately: the clause was contradicted by the code
   it was written to describe, and both had been in the repository, side by side, for ten
   rounds.**
